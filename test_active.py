...
def run_test():
    config = load_config()
    active = config.get("strategy", {}).get("active", "mean_reversion")
    strategy_config = config.get("strategy", {}).get(active, {})

    print(f"\n{'='*80}")
    print(f"  PIPST4EET - {active.upper()} STRATEGY")
    print(f"{'='*80}")
    
    print(f"\n  CONFIG:")
    print(f"    Pairs: {config['trading']['pairs']}")
    print(f"    SL: {strategy_config.get('atr_multiplier_sl', 1.0)}x ATR")
    print(f"    TP: {strategy_config.get('atr_multiplier_tp', 1.5)}x ATR")
    print(f"    R:R: 1:{strategy_config.get('atr_multiplier_tp', 1.5)/strategy_config.get('atr_multiplier_sl', 1.0):.1f}")
