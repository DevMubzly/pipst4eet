...
def run_test():
    ...
    print(f"\n{'='*80}")
    print(f"  PIPST4EET - TREND PULLBACK STRATEGY")
    print(f"{'='*80}")
    
    tp_config = config.get("strategy", {}).get("trend_pullback", {})
    
    print(f"\n  CONFIG:")
    print(f"    Pairs: {config['trading']['pairs']}")
    print(f"    R:R: 1:{tp_config.get('atr_multiplier_tp', 2.25)/tp_config.get('atr_multiplier_sl', 1.5):.1f}")
    print(f"    SL: {tp_config.get('atr_multiplier_sl', 1.5)}x ATR")
    print(f"    TP: {tp_config.get('atr_multiplier_tp', 2.25)}x ATR")
    print(f"    Timeframe: {config['trading']['timeframe']}")
    print(f"    Fast EMA: {tp_config.get('fast_ema_period', 20)}")
    print(f"    Med EMA: {tp_config.get('med_ema_period', 50)}")
    print(f"    Slow EMA: {tp_config.get('slow_ema_period', 100)}")
    ...
    for symbol in pairs:
        ...
        risk = RiskManager(config, total_balance)
        ...
        for i in range(len(df)):
            ...
            in_session = (config["trading"]["session_start"] <= time_str <= config["trading"]["session_end"])
        ...
        total_balance = engine.balance = risk.balance
    ...
    print(f"  Return:                        {(total_balance-initial_balance)/initial_balance*100:.1f}%")
    ...
    print(f"\n  All Trades:")
    print(f"\n  {'#':<4} {'Date':<12} {'Pair':<8} {'Dir':<4} {'Entry':>10} {'Exit':>10} {'PnL':>10} {'R':>8} {'Reason':<15}")
    print(f"  {'-'*4} {'-'*12} {'-'*8} {'-'*4} {'-'*10} {'-'*10} {'-'*10} {'-'*8} {'-'*15}")
